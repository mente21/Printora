...
function doReplace(regex, replacement, name) {
    if (code.match(regex)) {
...
if (!code.includes('const [bannerRealW')) {
    doReplace(stateRegex, `const [isSaving, setIsSaving] = useState(false);
    // Banner State
...
doReplace(canvasSizeRegex, `const canvasSize = selectedProduct.id === 'ceramic-mug' 
        ? { width: 1024, height: 512 } 
        : selectedProduct.id === 'banner'
        ? { width: Math.max(500, bannerRealW), height: Math.max(500, bannerRealH) }
        : { width: 500, height: 540 };`, 'Canvas Size');

// 3. Inject BannerMockup
...
`;
    doReplace(/export default function EditorUI/, bannerComponents + '\nexport default function EditorUI', 'Banner Mockup');
}

// 4. Update renderMockup
const renderMockupRegex = /case 'ceramic-mug': return <MugMockup \{\.\.\.props\} \/>;/;
doReplace(renderMockupRegex, `case 'ceramic-mug': return <MugMockup {...props} />;
            case 'banner': return <BannerMockup {...props} bannerRealW={bannerRealW} bannerRealH={bannerRealH} />;`, 'Render Mockup');

// 5. Replace Right Panel
...
doReplace(rightPanelRegex, rightPanelReplacement, 'Right Panel');

// 6. Fix handleOrderClick Loading Loop
const handleOrderClickRegex = /const handleOrderClick = async \(\) => \{[\s\S]*?setIsSaving\(false\);[\s\S]*?\} catch \(e\) \{[\s\S]*?setIsSaving\(false\);[\s\S]*?\}[\s\S]*?\};/;
doReplace(handleOrderClickRegex, `const handleOrderClick = async () => {
        setIsSaving(true);
        try {
    ...
    };`, 'handleOrderClick Fix');

// 7. Update finalVariants
const finalVariantsRegex = /const finalVariants = \{[\s\S]*?receiptDataUrl: receiptDataUrl \|\| originalOrder\?\.variants\?\.receiptDataUrl \|\| null,[\s\S]*?\};/;
doReplace(finalVariantsRegex, `const finalVariants = {
            ...
            };`, 'finalVariants Update');

// 8. Modal Customization
doReplace(/<h3 className="text-lg font-bold text-gray-900">Choose Size & Quality<\/h3>/, `<h3 className="text-lg font-bold text-gray-900">{selectedProduct.id === 'banner' ? 'Confirm Dimensions' : 'Choose Size & Quality'}</h3>`, 'Modal Title');

const modalGridRegex = /<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">\s*<div className="space-y-3">\s*<label className="text-\[11px\] font-black text-gray-400 uppercase tracking-widest ml-1">Select Size<\/label>[\s\S]*?<\/div>\s*<\/div>/;
const bannerModalGrid = `
                                {selectedProduct.id === 'banner' ? (
...
doReplace(modalGridRegex, bannerModalGrid, 'Modal Grid');
